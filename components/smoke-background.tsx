...
class Renderer {
  ...
  private color: [number, number, number] = [0.16, 0.72, 0.71];
  ...
  render(now = 0) {
    ...
    gl.uniform2f(
      (program as unknown as Record<string, WebGLUniformLocation>).resolution,
      canvas.width,
      canvas.height
    );
    gl.uniform1f(
      (program as unknown as Record<string, WebGLUniformLocation>).time,
      now * 1e-3
    );
    gl.uniform3fv(
      (program as unknown as Record<string, WebGLUniformLocation>).u_color,
      this.color
    );
